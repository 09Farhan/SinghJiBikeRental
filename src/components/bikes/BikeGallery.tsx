  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className={cn(
        ...
        {hasImages ? (
          <img src={images[selectedIndex]} alt={`${bikeName} - ${selectedIndex + 1}`} className="w-full h-full object-cover" />
        ) : (
          <>
      ...
      <div className="grid grid-cols-3 gap-4">
        {displayItems.map((item, idx) => (
          ...
          >
            {hasImages ? (
              <img src={item} alt={`${bikeName} thumbnail ${idx + 1}`} className="w-full h-full object-cover" />
            ) : (
